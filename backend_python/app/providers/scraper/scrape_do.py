...
class ScrapeDoProvider(ScraperProvider):
    ...
    async def fetch_page(self, url: str, api_key: Optional[str] = None, base_url: Optional[str] = None) -> str:
        if not api_key:
            raise ValueError("Scrape.do requires an API token.")
        
        encoded_target = urllib.parse.quote(url, safe="")
        endpoint = f"https://api.scrape.do?token={api_key}&url={encoded_target}"
    ...
    async def test_connection(self, api_key: str, base_url: Optional[str] = None) -> bool:
        ...
        try:
            ...
            endpoint = f"https://api.scrape.do?token={api_key}&url={test_target}"
